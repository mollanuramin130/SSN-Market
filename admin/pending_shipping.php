...
<section class="content-header">
    <div class="content-header-container" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
        <!-- Back button on the left -->
        <div class="content-header-right">
            <a href="pending_orders.php" class="btn btn-primary">P. Order</a>
        </div>

        <!-- Centered title -->
        <div class="content-header-center" style="flex-grow: 1; text-align: center;">
            <h4 style="color:red;"><strong>PENDING SHIPPING</strong></h4>
        </div>

        <!-- Forward button on the right -->
        <div class="content-header-right">
            <a href="order.php" class="btn btn-primary">M. Order</a>
        </div>
    </div>
</section>
        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
            	<?php
                $i = 0;
                // Modify the query to only fetch rows where payment_status is 'Pending'
                $statement = $pdo->prepare("SELECT * FROM tbl_payment WHERE shipping_status = 'Pending' AND payment_status = 'Completed' ORDER BY id DESC");
                $statement->execute();
                $result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr class="<?php if($row['payment_status'] == 'Pending') {
                            echo 'bg-r';
                        } elseif($row['shipping_status'] == 'Pending') {
                            echo 'bg-gray';
                        } else {
                            echo 'bg-g';
                        } 
                    ...
                    >
                        ...
                        <td>
                           <?php
                                ...
                                $statement50 = $pdo->prepare("SELECT * FROM tbl_customer WHERE cust_id=?");
                                $statement50->execute(array($row['customer_id']));
                                $result50 = $statement50->fetchAll(PDO::FETCH_ASSOC);							
                                foreach ($result50 as $row50) {
                                    echo '<b>Customer Phone:</b><br><a href="tel:'.$row50['cust_phone'].'">'.$row50['cust_phone'].'</a>';
                                    echo '<br><b>Delivery Name:</b><br>'.$row50['cust_s_name'];
                                    echo '<br><b>Delivery Phone:</b><br><a href="tel:'.$row50['cust_s_phone'].'">'.$row50['cust_s_phone'].'</a>';
                                    echo '<br><b>Address:</b><br>';
                                    
                                    // Fetch country name using cust_country from tbl_customer
                                    $statement51 = $pdo->prepare("SELECT * FROM tbl_country WHERE country_id=?");
                                    $statement51->execute(array($row50['cust_country']));
                                    $result51 = $statement51->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($result51 as $row51) {
                                        echo $row51['country_name'].'<br>';
                                    }
                                    
                                    // Continue printing other address details
                                    echo $row50['cust_address'].'<br>'.$row50['cust_city'].'<br>'.$row50['cust_state'].'<br>'.$row50['cust_zip'];
                                }
                            ?>
                        ...
                        <td>
                            ...
                            if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_status'])) {
                                ...
                                $result = $statement->execute([$new_status, $payment_id]);
                        
                                if ($result) {
                                    echo "<br><b>Current Status: </b>".$row['order_status'];
                                } else {
                                    echo "Failed to update order status.";
                                }
                            }
                        
                            // Assuming $row['payment_id'] is the current payment record's ID
                            ?>
                            
                            <form method="post">
                                <input type="hidden" name="payment_id" value="<?php echo $row['payment_id']; ?>">
                                <label for="order_status">Update Order Status:</label>
                                <textarea name="order_status" id="order_status" rows="6" cols="10"><?php echo htmlspecialchars($row['order_status']); ?></textarea>
                                <br>
                        ...
                        <td >
                                            ...
                                            foreach ($result1 as $row1) {
                                                ...
                                                foreach ($result2 as $row2) {
                                                    ...
                                                    echo '<img src="' . $imagePath . '" alt="Product Image" style="max-width: 100px; height: auto; border: 2px solid #ccc; padding: 5px; border-radius: 10px;">';
                                                    echo '<br><br>';
